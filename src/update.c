...
int hit_gain( CHAR_DATA *ch )
{
	...
	if ((conamount = (get_curr_con(ch)+1)) > 1)
	{
	    ...
	    {
		    gain *= conamount * ch->level * 2 ;	break;
		}
		else 
		{
			gain *= conamount * 0.5;	break;
		}
...
int mana_gain( CHAR_DATA *ch )
{
	...
	if ((intamount = (get_curr_int(ch)+1)) > 1)
	{
	    ...
	    {
		    gain *= intamount * ch->level * 2;	break;
		}
		else 
		{
			gain *= intamount * 0.5;	break;
		}
...
int move_gain( CHAR_DATA *ch )
{
	...
	if ((dexamount = (get_curr_dex(ch)+1)) > 1)
	{
	    ...
	    if ( IS_CLASS(ch, CLASS_NINJA) )
		{
		  gain *= dexamount * (ch->level * 2);	break;
		}
		else 
		{
		  gain *= dexamount * 0.5;	break;
		}
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	if ( ch->pcdata->stats[UNI_GEN] == 2 )
		...
		ch->pcdata->condition[iCond]	= URANGE( 0, condition + value, 180 );
	else if ( ch->pcdata->stats[UNI_GEN] == 4 )
		ch->pcdata->condition[iCond]	= URANGE( 0, condition + value, 150 );
...
*/
	if ( !IS_NPC(ch) )
	{
	    ...
	    IS_SET(ch->special, SPC_CHAMPION)) && IS_HERO(ch)
	    ...
	    && ch->in_room != NULL && ch->in_room->vnum == 20002)
	    {
...
/*
	    ...
	    IS_ITEMAFF(wch, ITEMA_STALKER) )
	{
		...
		|| wch->pcdata->powers[NPOWER_CHIKYU] > 3)
	    {
