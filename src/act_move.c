...
void move_char( CHAR_DATA *ch, int door )
{
	...
	{
	    if ( !IS_AFFECTED(ch, AFF_FLYING) && (!IS_NPC(ch) 
...
{
	if ( IS_NPC(ch) ) return;
    if (!IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_MONK))
    {
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    else if (!str_cmp(arg1,"silver") && IS_CLASS(ch,CLASS_WEREWOLF)) is_ok = TRUE;  
    else if (!str_cmp(arg1,"supremely") && IS_CLASS(ch,CLASS_WEREWOLF) ) is_ok = TRUE;
    else if (!str_cmp(arg1,"ancilla") && IS_CLASS(ch,CLASS_VAMPIRE)) is_ok = TRUE;
    else if (!str_cmp(arg1,"elder") && IS_CLASS(ch,CLASS_VAMPIRE)) is_ok = TRUE;
    else if (!str_cmp(arg1,"methuselah") && IS_CLASS(ch,CLASS_VAMPIRE)) is_ok = TRUE;
    else if (!str_cmp(arg1,"magic") && IS_CLASS(ch,CLASS_DROW)) is_ok = TRUE;
    ...
    else if ( !str_cmp( arg1, "avatar") && (ch->level == 2 || IN_TROOM(ch)))
    {
        cost        = 1000;
	pAbility    = &ch->level;
	pOutput     = "level";
    ...
    else if (!str_cmp( arg1, "supremely") && 
    ...
    if (ch->exp < 5000000)
    {
     ...
     ch->wpn[0] = 250;
     ch->exp = ch->exp - 5000000;
     send_to_char( "Ok.\n\r", ch);
     return;
    }
    
/* Vampire Age Training - Loki */
	...
	if ( IS_CLASS(ch, CLASS_WEREWOLF) && ch->wpn[0] < 201 )
	{
	  sprintf( buf, "Supremely - Become supremely skilled in unarmed combat. 5 Million Exp.\n\r");
	  send_to_char( buf, ch );
	}
	
/* Show age training in train to vampires - Loki */	
	...
	if ( IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->rank < AGE_METHUSELAH
	     && AGE_ELDER && ch->pcdata->rank > AGE_ANCILLA)
	{
