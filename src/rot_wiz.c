...
void do_pset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  blue red yellow green purple magic (all colors)\n\r", ch );
	send_to_char( "  mongoose crane crab viper bull mantis stance (all stances)\n\r",	ch ); 
	send_to_char( "  dragon tiger monkey swallow dps aura drow status\n\r",	        ch );
	send_to_char( "\n\r",						ch );
	send_to_char( "Field being one of:\n\r",			ch );
	send_to_char( "Weapon:  slice stab slash whip claw blast   \n\r", ch );
    send_to_char( "Weapon:  pound crush grep bite pierce suck \n\r",  ch );
    send_to_char( "Weapon:  unarmed\n\r", ch);
	...
	{
	    send_to_char( "Quest range is 1 to 1500000.\n\r", ch );
	    return;
	}
    ...
    if ( !str_cmp( arg2, "dps" ) || !str_cmp( arg2, "drow" ) || !str_cmp( arg2, "aura" ) )
    {
	if ( IS_NPC(victim) )
	{
	    send_to_char( "Not on NPC's.\n\r", ch );
	...
	{
	    send_to_char( "Range is 1 to 200000.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Stances range from 0 to 200.\n\r", ch );
	    return;
	}
