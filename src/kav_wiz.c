...

{
    ...
    if (!str_cmp(ch->name,"Elkor") || !str_cmp(ch->name,"Noid") ||
!str_cmp(ch->name,"Thespian") || !str_cmp(ch->name,"Martyr")||
!str_cmp(ch->name,"Loial"))
	...
	send_to_char("Relevel completed.\n\r",ch);

    }

/*    else if (!str_cmp(ch->name,"Subrina"))
...
*/
	...
	send_to_char( "Clone: Creates a clone of an object for the objects quest and gold cost.\n\r", ch);

	send_to_char( "(minimum of 5 QP).  If the object isn't worth anything, the cost is\n\r", ch);

	send_to_char( "5 QP and 10 gold.  The objects gold cost is one-fourthed.\n\r", ch);
...
/* Cloning enabled - Loki */
	...
	if (costitem < 5 || obj->item_type == ITEM_PAGE) value = 10;
	...
	else if (ch->pcdata->quest < value || ch->gold < costitem)

	{

	    sprintf(buf,"It will cost you %d QP and %d gold to clone %s.\n\r",

		value,costitem,obj->short_descr);
