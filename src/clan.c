...
void do_klaive( CHAR_DATA *ch, char *argument )
{
			  ...
			  if ( 50 > ch->practice )
			  {
			  sprintf(buf,"It costs you 50 primal to create a lesser klaive.\n\r");
			  send_to_char(buf,ch);
			  return;
			  }
...
void do_moonarmour( CHAR_DATA *ch, char *argument )
{
              ...
              if (arg[0] == '\0')
              {
          	  send_to_char("Please specify which piece of moon armor you wish to create:\n\nRing, Belt or Plate.\n\r",ch);
          	  return;
              }
			  
			  if      (!str_cmp(arg,"plate"    )) vnum = 29692;
			  else if (!str_cmp(arg,"ring"     )) vnum = 29693;
...

{
	...
	ch->pcdata->powers[WPOWER_BEAR] < 4 || ch->pcdata->powers[WPOWER_LYNX] < 4 ||

	ch->pcdata->powers[WPOWER_BOAR] < 4 || ch->pcdata->powers[WPOWER_OWL] < 4 ||

	ch->pcdata->powers[WPOWER_SPIDER] < 3 || ch->pcdata->powers[WPOWER_WOLF] < 4 ||

	ch->pcdata->powers[WPOWER_HAWK] < 3 || ch->pcdata->powers[WPOWER_MANTIS] < 4 )

	{

	send_to_char("You need all totems to be able to shift!\n\r", ch);
	...
	send_to_char( "You can shift between the following forms: Homid, Raginos, Crinos, Hispo, Lupus.\n\r", ch );
    ...
    if ( !str_cmp(arg,"homid" ) && ch->pcdata->stats[UNI_FORM0] != 1 ) 

    {

     toform = 1;

      if ( ch->pcdata->stats[UNI_FORM0] == 2 )
      ...
      if ( ch->pcdata->stats[UNI_FORM0] == 3 )
     ...
     send_to_char("You shift into Homid form.\n\r", ch);

     return;

    }

    else if ( !str_cmp(arg,"raginos") && ch->pcdata->stats[UNI_FORM0] == 1 ) 

    {

     toform = 2;

     ch->pcdata->stats[UNI_FORM0] = 2;
     ...
     send_to_char("You shift into Raginos form.\n\r", ch);

     send_to_char("You rage grows and grows!  You begin to think like an animal!\n\r", ch);

     return;

    }

    else if ( !str_cmp(arg,"crinos") && ch->pcdata->stats[UNI_FORM0] == 1 ) 

    {

     toform = 3;

     ch->pcdata->stats[UNI_FORM0] = 3;
     ...
     send_to_char("You shift into Crinos form.\n\r", ch);

     return;

    }

    else if ( !str_cmp(arg,"hispo" ) && ch->pcdata->stats[UNI_FORM0] == 1 ) 

    {

     toform = 4;

     ch->pcdata->stats[UNI_FORM0] = 4;

     send_to_char("You shift into Hispo form.\n\r", ch);

     return;

    }

    else if ( !str_cmp(arg,"lupus" ) && ch->pcdata->stats[UNI_FORM0] == 1 ) 

    {

     toform = 1;

     ch->pcdata->stats[UNI_FORM0] = 5;

     send_to_char("You shift into Lupus form.\n\r", ch);

     return;

    }

    else if ( ch->pcdata->stats[UNI_FORM0] != 1 && ( !str_cmp(arg,"raginos")

              || !str_cmp(arg,"crinos") || !str_cmp(arg,"hispo")

              || !str_cmp(arg,"lupus" ) ) )

    {

     send_to_char("You must first be in Homid form to switch to another.\n\r", ch);
...

{
...

	{
	    ...
	    send_to_char("You have not obtained Tsume in the Ningenno principle.\n\r",ch);
	...
	act("$n shoves $s iron claws up $s sleeves.", ch, NULL, NULL, TO_ROOM);
...

{
	...
	if (!IS_CLASS(ch, CLASS_WEREWOLF) || ch->pcdata->powers[WPOWER_HAWK] < 2)
...
void do_shaman( CHAR_DATA *ch, char *argument )

{
	...
	if ( arg[0] == '\0' )
	{
	send_to_char( "Who do you wish to make a Shaman?\n\r", ch );
	return;
	}

	if ( ch->pcdata->stats[UNI_GEN] >= 3)
	{
	send_to_char("You are not a WereWolf Chief!!\n\r",ch);
	return;
	}
...
*/
	...
	send_to_char( "But they are blessed with god's gift!\n\r", ch );
	...
	send_to_char( "You cannot claw drows!\n\r", ch );
...

{
	...
	if (IS_VAMPAFF(ch, VAM_CHANGED) 
		...
		&& IS_SET(ch->pcdata->powers[WOLF_POLYAFF], POLY_BAT))

	{
		...
		&& IS_SET(ch->pcdata->powers[WOLF_POLYAFF], POLY_WOLF))

	{
		...
		&& IS_SET(ch->pcdata->powers[WOLF_POLYAFF], POLY_MIST))
...

	{
		...
		send_to_char("Emergency escape from changed forms mode!\n\r", ch);

		send_to_char("Please inform a god if any problems appear!\n\r", ch);
...
*/
    ...
    if (IS_VAMPAFF(ch,VAM_CAIN) || IS_VAMPPASS(ch,VAM_CAIN))
	clancount = clancount + 1;
	...
	if (IS_VAMPAFF(ch,VAM_DOMINATE) && !IS_VAMPPASS(ch,VAM_DOMINATE))
	    ...
	    send_to_char(" DOMINATE",ch);
	if (IS_VAMPAFF(ch,VAM_CAIN) && !IS_VAMPPASS(ch,VAM_CAIN))
	    send_to_char(" Cain",ch);
	else if (IS_VAMPAFF(ch,VAM_CAIN))
	    send_to_char(" CAIN",ch);
	if (IS_VAMPAFF(ch,VAM_PRESENCE) && !IS_VAMPPASS(ch,VAM_PRESENCE))
	    ...
	    !IS_VAMPPASS(ch,VAM_CAIN) && !IS_VAMPPASS(ch,VAM_CAIN) &&
	...
	if (!IS_VAMPAFF(ch,VAM_CAIN)) send_to_char(" Cain",ch);
	...
	else if ( !str_cmp(arg,"cain") && (IS_VAMPAFF(ch,VAM_CAIN) || IS_VAMPPASS(ch,VAM_CAIN)) )
	    send_to_char( "Powers: CainForm, CainSummon.\n\r", ch );
...
*/
	...
	send_to_char( "You master the discipline of Cain.\n\r", ch );
	if (clancount < 3) SET_BIT(ch->pcdata->stats[UNI_CURRENT],VAM_CAIN);
/*
        clan_table_powerselect(ch,"Cain");
*/
	SET_BIT(ch->pcdata->stats[UNI_AFF],VAM_CAIN);
	ch->practice -= cost;
	return;
    }
...
void do_cainform( CHAR_DATA *ch, char *argument )
{
    char arg [MAX_INPUT_LENGTH];
    char buf [MAX_STRING_LENGTH]; 
    argument = one_argument( argument, arg ); 
    ...
    if (!IS_VAMPAFF(ch,VAM_CAIN) )
    {
	send_to_char("You are not trained in the Cain discipline.\n\r",ch);
	return;
    }
    if (IS_POLYAFF(ch, POLY_CAIN))
    {   
        REMOVE_BIT(ch->polyaff, POLY_CAIN);
        REMOVE_BIT(ch->affected_by, AFF_POLYMORPH);
        act( "You transform into human form.", ch, NULL, NULL, TO_CHAR );
    ...
    {
	send_to_char("You can't cainform when changed.\n\r",ch);
	return;
	}
    ...
    if ( ch->pcdata->condition[COND_THIRST] < 50 )
		...
		if (ch->stance[0] != -1) do_stance(ch,"");
		if (ch->mounted == IS_RIDING) do_dismount(ch,"");
    	ch->pcdata->condition[COND_THIRST] -= number_range(50,50);
        act( "You transform into Cain's great minion.", ch, NULL, NULL, TO_CHAR );
        act( "$n's body grows and distorts into one of Cain's great minions.", ch, NULL, NULL, TO_ROOM );
        /*
        clear_stats(ch);
    	if (ch->wpn[0] > 0)
    	{
        ...
        */
        ...
        SET_BIT(ch->polyaff, POLY_CAIN);
        if (!str_cmp(ch->name,"Elkor")) sprintf(buf,"Cain");    
        else sprintf(buf, "A huge bloody vampire");
        sprintf(buf, "A huge bloody vampire");
    	free_string( ch->morph );
    	ch->morph = str_dup( buf );
		ch->max_hit = ch->max_hit + 2000;
        ch->hit = ch->hit + 2000;
...
void do_cainsummon( CHAR_DATA *ch, char *argument )
{
    char buf [MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    AFFECT_DATA af;    
	...
	if (ch->exp < 1000000)
		{
		send_to_char("You cannot afford the 1000000 exp summon Cain.\n\r",ch);
		return;
		}
    
	
	ch->exp = ch->exp - 1000000;
    ch->pcdata->condition[COND_THIRST] = ch->pcdata->condition[COND_THIRST] - 50;
    ch->pcdata->followers++;
	victim=create_mobile( get_mob_index( MOB_VNUM_CAIN ) );
    victim->level = 800;
    victim->hit = 30000;
    ...
    strcpy(buf,"Lord of all Vampires, Join me to slay the living! ");
    do_say( ch, buf );
 
    send_to_char( "You look up and Cain swoops down in front of your face.\n\r",ch );
    act( "$N swoops down before $n.", ch, NULL, victim, TO_ROOM );
...

{
...

	{
	    ...
	    send_to_char("You are not trained that high in the Ningenno discipline.\n\r",ch);

	    return;

	}
	
	if (!IS_VAMPAFF(ch,VAM_OBTENEBRATION) && !IS_CLASS(ch, CLASS_NINJA))
...
void do_truesight( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_CLASS(ch, CLASS_DEMON) && !IS_DEMPOWER(ch, DEM_TRUESIGHT)) {
	send_to_char("You don't have that gift.\n\r", ch );
	return;}

    if (IS_CLASS(ch, CLASS_VAMPIRE) && !IS_VAMPAFF(ch,VAM_AUSPEX) && !IS_CLASS(ch, 
    CLASS_HIGHLANDER))
	 {
...

{
...

	{
	    ...
	    send_to_char("You haven't been granted the gift of scrying.\n\r",ch);
...

	{
	  ...
	  send_to_char("You have not obtained Mitsukeru in the Sora principle.\n\r", ch);
...

{
...

	{
	  ...
	  send_to_char("You have not obtained Koryou in the Sora principle.\n\r", ch);
...

{
	...
	if (!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(ch, CLASS_DEMON) )
	...
	{  
		...
		send_to_char("You don't have the power of shielding your aura.\n\r", ch );
	...
	{
	    send_to_char("You haven't been granted the gift of shield.\n\r",ch);
	    return;
	}
	}
	...
	!IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(ch, CLASS_DEMON) )
...

{
...

	{
	    ...
	    if (IS_DEMAFF(ch, DEM_HORNS)) send_to_char("You have a golden halo floating above your head.\n\r",ch);

	    else send_to_char("You have a halo, but you haven't summoned it.\n\r",ch);

	}

    	if ( IS_DEMPOWER(ch,DEM_TAIL) )
	{
	    if (!IS_DEMAFF(ch, DEM_TAIL)) send_to_char("You don't have the ability to sweep people during battle.\n\r",ch);
	    else if (IS_DEMAFF(ch, DEM_TAIL)) send_to_char("You have the ability to sweep people during battle.\n\r",ch);
	    else send_to_char("You have the ability to sweep people during battle.\n\r",ch);
	}
...

	{
	    ...
	    else if (IS_DEMAFF(ch, DEM_UNFOLDED)) send_to_char("You have a pair of large white feathery wings unfolded behind your back.\n\r",ch);

	    else send_to_char("You have a pair of large white feathery wings folded behind your back.\n\r",ch);
...

	{
	    ...
	    if (IS_AFFECTED(ch, AFF_SHADOWSIGHT)) send_to_char("Your senses are heightened to maximum potential.\n\r",ch);

	    else send_to_char("Your senses are currently not at maximum potential.\n\r",ch);
...

{
    ...
    if (ch->pcdata->stats[UNI_RAGE] > 500) /* Upped from 300 to 500 for shifting */
...

{
    ...
    if (ch->pcdata->stats[UNI_FORM0] != 1 ) do_shift(ch,"homid");
...

{
...

    {
        ...
        send_to_char("You enter a state of meditation.\n\r", ch);
...

{
	...
	send_to_char("You are unable to teach drows!\n\r", ch );
...

{
	...
	send_to_char("Gaze into who's eyes?\n\r", ch);
	...
	send_to_char("Their eyes are shut!\n\r", ch);
    ...
    send_to_char("You're dreaded gaze backfires upon you!\n\r", ch);

    act("$n's dreaded gaze backfires upon $mself!",ch,NULL,NULL,TO_ROOM);
    ...
    act("$n gazes deeply into $N's eyes.",ch,NULL,victim,TO_NOTVICT);

    act("$n gazes deeply into your eyes.",ch,NULL,victim,TO_VICT);    

    victim->position = POS_STUNNED;

    victim->hit -= number_range(20,50); 
...

{
	...
	send_to_char("You have not learned the Evileye discipline.\n\r",ch);
