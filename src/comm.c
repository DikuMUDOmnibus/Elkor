...
/*
	...
	** Avoid nasty duplication bug - KaVir */
	...
	if (ch->desc != NULL && ch->desc->host != NULL)
	{
	    ...
	    else if                                                            (!str_cmp(ch->name,"Tilly")) ch->lasthost = str_dup("paradise.com");
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\rWelcome to The Lords Of War Mud.  May thy blade stay ever
sharp, thy soul ever dark.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
...
*/
	...
	if (!str_cmp(ch->name,"Elkor") || !str_cmp(ch->name,"Robert") || !str_cmp(ch->name,"Veldran") || !str_cmp(ch->name,"IceCreamPimp"))
	...
	sprintf(buf,"%s has entered the Realms of Elkor.", ch->name);
	do_info(ch,buf);
