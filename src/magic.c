...

{
	...
	if (IS_AFFECTED(victim, AFF_DROWBLESS)) bless = TRUE;
	...
	if (dark == TRUE) REMOVE_BIT(victim->in_room->room_flags, ROOM_TOTAL_DARKNESS);
	...
	SET_BIT(victim->affected_by, AFF_DROWBLESS);
...

{
	...
	if (IS_SET(obj->spectype, SITEM_NINJA))

	send_to_char("This item was crafted by the art of ninjitsu.\n\r", ch);
...

{
    ...
    && victim->pcdata->powers[NPOWER_NINGENNO] > 4 ) return;
...

{
    ...
    /* Mounts attack caster if there are more then 4 people following the person - Loki */

    victim=create_mobile( get_mob_index( MOB_VNUM_GUARDIAN ) );

    victim->level = level;

    victim->hit = 200*level;

    victim->max_hit = 200*level;

    victim->hitroll = level*2;

    victim->damroll = level*2;

    victim->armor = 100 - (level*20);
	...
	strcpy(buf,"I will never follow you!  DIE!!!");
...

{
    ...
    if (ch->pcdata->followers > 4 && IS_CLASS(ch, CLASS_DEMON) )
	...
	victim->hitroll = level*4;

	victim->damroll = level*4;
	...
	strcpy(buf,"You dare to call upon too many of Hells minions!  DIE!!!");
    ...
    else if (ch->pcdata->followers > 4 && IS_CLASS(ch, CLASS_DROW) )

    {

	victim=create_mobile( get_mob_index( 30302 ) );
	...
	strcpy(buf,"You dare to call upon too many of Lloths minions!  DIE!!!");
...

	{
        ...
        victim=create_mobile( get_mob_index( 29600 ) );
...

{
    ...
    if ( familiar->in_room != ch->in_room )

    {

    act("With a gesture, you summon your familiar.", ch,NULL,NULL,TO_CHAR);

    act("With a gesture, $n summons $s familiar.", ch,NULL,NULL,TO_ROOM);

    char_from_room(familiar);
	...
	send_to_char( "Your familiar is standing right here!\n\r", ch );
...
void spell_drowbless( int sn, int level, CHAR_DATA *ch, void *vo)

{
	...
	act("$n is blessed by Lloth!",ch,NULL,NULL,TO_ROOM);

	SET_BIT( ch->affected_by, AFF_DROWBLESS );
