...
DECLARE_SPEC_FUN(   spec_clan_mage  ); /* Mage Guardian */

DECLARE_SPEC_FUN(   spec_clan_demon ); /* Demon Guardian */

DECLARE_SPEC_FUN(   spec_executioner ); /* Executioner */

DECLARE_SPEC_FUN(   spec_healer     ); /* Healer */

DECLARE_SPEC_FUN(   spec_clan_gangrel); /* Gangrel Vampire Clan */

DECLARE_SPEC_FUN(   spec_clan_tremere); /* Tremere Vampire Clan */

DECLARE_SPEC_FUN(   spec_clan_ninja); /* Ninja Clan */

DECLARE_SPEC_FUN(       spec_clan_silver_fangs   );

DECLARE_SPEC_FUN(       spec_clan_black_spiral_dancers );

DECLARE_SPEC_FUN(   spec_clan_wu_tang );

DECLARE_SPEC_FUN(   spec_clan_red_talons );
...
/*    if ( !str_cmp( name, "spec_clan_werewolf"     ) ) return spec_clan_werewolf;*/
    ...
    if ( !str_cmp( name, "spec_clan_black_spiral_dancers"     ) ) return spec_clan_black_spiral_dancers;
    ...
    if ( !str_cmp( name, "spec_clan_gangrel" ) ) return spec_clan_gangrel;

    if ( !str_cmp( name, "spec_clan_tremere" ) ) return spec_clan_tremere;

    if ( !str_cmp( name, "spec_clan_ninja"  ) ) return spec_clan_ninja;

    if ( !str_cmp( name, "spec_clan_wu_tang" ) ) return spec_clan_wu_tang;

	if ( !str_cmp( name, "spec_clan_red_talons" ) ) return spec_clan_red_talons;
...
|| victim->pcdata->chobj != NULL
          ...
          /* || (IS_CLASS(victim, CLASS_WEREWOLF))*/
           ...
           || (victim->clan != NULL && !strcmp(victim->clan,"Black Spiral
Dancers") ))
...
bool spec_clan_red_talons(CHAR_DATA *ch)
...
|| victim->pcdata->chobj != NULL
          ...
          /* || (IS_CLASS(victim, CLASS_WEREWOLF))*/
           ...
           || (victim->clan != NULL && !strcmp(victim->clan,"Red Talons") ))
...
headquarters!",ch,NULL,victim,TO_CHAR);
 ...
 char_to_room(victim, get_room_index(ROOM_VNUM_ALTAR));
        ...
        sprintf( buf,"%s Is in Red Talons Head Quarters! Attack!\n\r",
...
bool spec_clan_wu_tang( CHAR_DATA *ch )

{
...

   {
       ...
       if (!IS_PLAYING(d)
        ...
        /* || (IS_CLASS(victim, CLASS_WEREWOLF))*/
           ...
           || (victim->clan != NULL && !strcmp(victim->clan,"Wu-Tang")))
 ...
 char_to_room(victim, get_room_index(ROOM_VNUM_ALTAR));
        ...
        sprintf( buf,"%s Is in the Wu-Tang Quarters! Attack!\n\r", victim->name);
...

{
...

   {
...

	{
        ...
        sprintf( buf,"%s is in Werewolf Headquarters! Attack!\n\r", victim->name); 
...

{
...

   {
	   ...
	   || (victim = d->character) == NULL
        ...
        /* Stop Cerberus from attack people in rooms other then 30000 - Vic */

        if (ch->in_room != victim->in_room

         && victim->in_room->vnum == 30000 )

        {

           act("$n is engulfed in flames and vanishes into the ground!",victim,NULL,NULL,TO_ROOM);

           act("You are engulfed in flames and vanish into the ground!",ch,NULL,NULL,TO_VICT);
           ...
           act("$n appears in a bright light!",victim,NULL,NULL,TO_ROOM);

           act("You appear in a bright light!",ch,NULL,NULL,TO_VICT);
        ...
        sprintf( buf,"%s is in the Realm of Heaven! Attack!\n\r",
...

	{
...

      {
...

        {
           ...
           act("$n disappears into a time/space portal!!",victim,NULL,NULL,TO_ROOM);

           act("You disappear into a time/space portal!",victim,NULL,NULL,TO_CHAR);
           ...
           act("$n appears out of a time/space portal!!",victim,NULL,NULL,TO_ROOM);

           act("You appear out of a time/space portal!!",victim,NULL,NULL,TO_CHAR);
        ...
        sprintf( buf,"%s is in the Tower of the Mages! Attack!\n\r",
...

{
	...
	if ( object->questmaker != NULL )

	    continue;

	    

	if ( !IS_SET(object->quest, QUEST_RELIC) )
	...
	strcpy(buf,"Look at this mess!");

	do_say(ch,buf);   

	do_sacrifice(ch,object->name);  

	strcpy(buf,"Keep this temple clean or get OUT!");
...

{
	...
	/* Kill the executioner if it dies */

	update_pos( ch );

	

	if (ch->hit < ch->max_hit && ch->position != POS_FIGHTING)
...

    {
		...
		else if (ch->hit < (ch->max_hit * 0.25) && number_percent() < 25)

		{

		    act("$n's eyes glow bright blue for a moment.",ch,NULL,NULL,TO_ROOM);

		    spell_heal( skill_lookup( "heal" ), ch->level*50, ch, ch );
...

		{
			...
			case 3:
				...
				spell_gas_breath( skill_lookup( "gas breath" ), ch->level*2, ch, victim );
				...
				spell_fire_breath( skill_lookup( "fire breath" ), ch->level*2, ch, victim );
				...
				spell_frost_breath( skill_lookup( "frost breath" ), ch->level*2, ch, victim );
				...
				spell_acid_breath( skill_lookup( "acid breath" ), ch->level*2, ch, victim );

				spell_lightning_breath( skill_lookup( "lightning breath" ), ch->level*2, ch, victim );
			...
			case 8:
			    ...
			    spell_harm( skill_lookup( "harm" ), ch->level*500, ch, victim );
...

		{
			...
			strcpy(buf,"Foolish mortal, how dare you attack the executioner!");
			...
			if (IS_NPC(victim))
			    ...
			    sprintf(buf,"I shall feast on your corpse for this, %s",victim->short_descr);

			else

			    sprintf(buf,"I shall feast on your corpse for this, %s",victim->name);
			...
			if (IS_NPC(victim))
			    ...
			    sprintf(buf,"%s will DIE!",victim->short_descr);

			else

			    sprintf(buf,"%s will DIE!",victim->name);
...
bool spec_clan_gangrel(CHAR_DATA *ch)

{
...

   {
	   ...
	   || (IS_CLASS(victim, CLASS_VAMPIRE) && IS_SET(victim->special, SPC_INCONNU))
           ...
           || (victim->clan != NULL && !strcmp(victim->clan,"Gangrel") ))

	{
...

        {
           ...
           act("$n disappears in a puff of smoke!",victim,NULL,NULL,TO_ROOM);

           act("You disappear in a puff of smoke!",ch,NULL,NULL,TO_VICT);
           ...
           act("$n appears in a puff of smoke!",victim,NULL,NULL,TO_ROOM);

           act("You appear in a puff of smoke!",ch,NULL,NULL,TO_VICT);
        ...
        sprintf( buf,"%s is in the Gangrel Headquarters! Attack!\n\r", victim->name); 
...
bool spec_clan_tremere(CHAR_DATA *ch)

{
...

   {
	   ...
	   || (IS_CLASS(victim, CLASS_VAMPIRE) && IS_SET(victim->special, SPC_INCONNU))
           ...
           || (victim->clan != NULL && !strcmp(victim->clan,"Tremere") ))

	{
...

        {
           ...
           act("$n disappears into dark light!",victim,NULL,NULL,TO_ROOM);

           act("You disappear into dark light!",ch,NULL,NULL,TO_VICT);
           ...
           act("$n appears from a dark light!",victim,NULL,NULL,TO_ROOM);

           act("You appear from a dark light!",ch,NULL,NULL,TO_VICT);
        ...
        sprintf( buf,"%s is in the Tremere Headquarters! Attack!\n\r", victim->name); 
...
bool spec_clan_ninja(CHAR_DATA *ch)
...

   {
...

	{
...

        {
           ...
           act("$n disappears into flashes of light!",victim,NULL,NULL,TO_ROOM);

           act("You disappear into flashes of light!",ch,NULL,NULL,TO_VICT);
           ...
           act("$n appears from flashes of light!",victim,NULL,NULL,TO_ROOM);

           act("You appear from flashes of light!",ch,NULL,NULL,TO_VICT);
        ...
        sprintf( buf,"%s is in the Ninja Dojo! Help!\n\r", victim->name); 
