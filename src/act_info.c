...

{
    ...
    else if ( obj->points > 0 && obj->points < 100 ) strcat(buf, "(Legendary) " );

    else if ( obj->points >= 100 && obj->points < 250 ) strcat(buf, "(Mystical) " );

    else if ( obj->points >= 250 ) strcat(buf, "(Mythical) " );
	...
	if ( obj->condition < 100) strcat(buf, " (broken)");
...

{
	...
	(IS_CLASS(ch, CLASS_VAMPIRE) || IS_CLASS(ch, CLASS_WEREWOLF)
        ...
        || IS_CLASS(ch, CLASS_MAGE) || IS_CLASS(ch, CLASS_NINJA)
...

	{
		...
		act( "...$N is surrounded by cascading torrents of energy.", ch,NULL,victim,TO_CHAR );

	    if ( IS_ITEMAFF(victim, ITEMA_FIRESHIELD) ) 

		act( "...$N is engulfed within a blaze of mystical flame.", ch,NULL,victim,TO_CHAR );

	    if ( IS_ITEMAFF(victim, ITEMA_ICESHIELD) ) 

		act( "...$N is ensphered by shards of glistening ice.", ch,NULL,victim,TO_CHAR );

	    if ( IS_ITEMAFF(victim, ITEMA_ACIDSHIELD) ) 

		act( "...$N is surrounded within a swirl of acid.", ch,NULL,victim,TO_CHAR );
    ...
    else if (victim->position == POS_STANDING && (!IS_NPC(victim) && IS_CLASS(victim, CLASS_DROW) 

    && IS_SET(victim->pcdata->powers[1], DPOWER_LEVITATION)))

	strcat(buf, " is levitating here" );
...

{
...

    {
		...
		if (IS_DEMAFF(victim, DEM_HORNS)) act("$N has a halo floating above $S head.",ch,NULL,victim,TO_CHAR);
...

		{
				...
				act("$N has a pair of large white feathery wings spread out from behind $S back.",ch,NULL,victim,TO_CHAR);

	    		else

			act("$N has a pair of large white feathery wings folded behind $S back.",ch,NULL,victim,TO_CHAR);
...

{
	...
	"You have %d out of %d points of angelic power stored.\n\r",
...
/* Added showing of damcap - Vic */

    sprintf( buf, "Hitroll: %d.  Damroll: %d.  Damcap: %d.", char_hitroll(ch), char_damroll(ch), ch->damcap[DAM_CAP] );
...

	{
	    ...
	    sprintf(buf,"You are wearing angelic armour:  Affects Hitroll and Damroll by +%d.\n\r",
...

{
	...
	"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\rThe PK MUD started up at %s\rThe system time is %s\r",
...

{
	...
	default: class = "Mortal       "; break;

	case MAX_LEVEL -  0:

		if (!str_cmp(wch->name,"Elkor")) class = "Head Guy      ";

                else if (!str_cmp(wch->name,"Martyr")) class =   "Head Coder    ";

		else if (!str_cmp(wch->name,"Skye")) class =   "Mortal Affairs";

		else if (!str_cmp(wch->name,"Noid")) class =   "The Void      ";

		else if (!str_cmp(wch->name,"Darr")) class =   "The Server    ";
	...
	case MAX_LEVEL -  1: 
		...
		if (!str_cmp(wch->name,"Cable")) class = "The Cable Guy ";
		else if (!str_cmp(wch->name,"Jizzrat")) class = "The Quest Man ";
	...
	case MAX_LEVEL - 12: class = "Newbie        "; break;
	...
	if ( ch->class > 0 || IS_IMMORTAL( ch ))

	  {
	    ...
	    if ( IS_CLASS(wch, CLASS_DEMON) && IS_SET(wch->special, SPC_DEMON_LORD) )
	    	...
	    	sprintf( kav, ". %sArch-Angel of %s%s", openb, wch->clan, closeb );
		...
		if (IS_SET(wch->special, SPC_PRINCE) && wch->sex == SEX_FEMALE)
		    ...
		    sprintf( kav, ". %sAngel of %s%s", openb, wch->clan, closeb );
		    ...
		    sprintf( kav, ". %sGuardian Angel of %s%s", openb, wch->clan, closeb );

		else

		    sprintf( kav, ". %sCherubim of %s%s", openb, wch->clan, closeb );
...

	    {
	    	...
	    	if (wch->pcdata->stats[UNI_GEN] == 2)
	    	    ...
	    	    sprintf( kav, ". %s%s Wolf%s", openb, wch->clan, closeb );

	    	else if (wch->pcdata->stats[UNI_GEN] > 3)

	    	    sprintf( kav, ". %s%s Pup%s", openb, wch->clan, closeb );
		...
		&& wch->clan != NULL && strlen(wch->clan) < 1 )
	    	...
	    	sprintf( kav, ". %sRonin Wolf%s", openb, closeb );
...

	    {
	    	...
	    	if (wch->pcdata->stats[UNI_GEN] == 2)
	    	    ...
	    	    sprintf( kav, ". %s%s Chunin%s", openb, wch->clan, closeb );

	    	else

	    	    sprintf( kav, ". %s%s Genin%s", openb, wch->clan, closeb );
...

{
	...
	if (itemtype == 4 || itemtype == 1)
	    ...
	    sprintf (buf, "This ancient item calls upon the forces of darkness.\n\r");
