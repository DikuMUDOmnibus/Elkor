...

{
	...
	send_to_char( "But they look so peaceful.\n\r", ch );
	...
	send_to_char( "You cannot attack while in your Steel Shield.\n\r", ch );
	...
	send_to_char( "You cannot attack them in their Steel Shield.\n\r", ch );
...
*/
	...
	if (IS_CLASS(ch, CLASS_DEMON)  && IS_DEMAFF(ch,DEM_HORNS) 
		...
		&& number_range(1,3) == 1) multi_hit( ch, victim, gsn_headbutt );

	if (IS_CLASS(ch, CLASS_DEMON)  && IS_DEMAFF(ch,DEM_TAIL) 

		&& number_range(1,3) == 1) multi_hit( ch, victim, gsn_sweep );
	
	if ( IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_NINGENNO] > 3

        && number_range(1,3) == 1) multi_hit( ch, victim, gsn_shiroken );
...

{
    ...
    else if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_CHIKYU] > 4 )

		count += 2;

	else if (IS_CLASS(ch, CLASS_DROW) 

	&& (IS_SET(ch->special, SPC_DROW_WAR) || ch->pcdata->stats[UNI_GEN] < 3))
...

{
	...
	ch->pcdata->powers[NPOWER_CHIKYU] > 1 )
...

{
	...
	else if (IS_CLASS(ch, CLASS_DROW))  max_dam = 2500;

	else if (IS_CLASS(ch, CLASS_MONK))  max_dam += ch->pcdata->stats[PMONK] * 3;
...

	{
		...
		if 		(ch->pcdata->stats[UNI_GEN] < 3) max_dam = 2000;

	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 1800;

	    else if (ch->pcdata->stats[UNI_GEN] < 5) max_dam = 1600;
...

{
	...
	if (IS_AFFECTED(ch, AFF_DARKNESS)) dark = TRUE;
	
	if (dark == TRUE) REMOVE_BIT(ch->in_room->room_flags, ROOM_TOTAL_DARKNESS);

	if (dark == TRUE) REMOVE_BIT(ch->affected_by, AFF_DARKNESS);

	if (dark == TRUE) REMOVE_BIT(victim->affected_by, AFF_DARKNESS);
...

	{
...

		{
			...
			ch->pcdata->stats[DEMON_CURRENT] +=victim->level*4/3;

			ch->pcdata->stats[DEMON_TOTAL]   +=victim->level*4/3;
...

{
	...
	send_to_char( "You are going to fight if you have implementor powers!\n\r", ch );
	...
	send_to_char( "You are going to fight someone with implementor powers!\n\r", ch );
...

{
	...
	if ( victim->pcdata->powers[PMONK] > 4 )
	...
	else if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_CHIKYU] > 2 )
	...
	else if (IS_CLASS(victim, CLASS_NINJA) && victim->pcdata->powers[NPOWER_CHIKYU] > 2 )
...

{
	...
	else if (IS_CLASS(ch, CLASS_MONK) && ch->pcdata->powers[PMONK] > 4 )

		chance -= (ch->pcdata->powers[PMONK] * 5 );
...

{
	...
	/* When mortals die, they get there corpses back - Loki */
...

{
    ...
    xp    = 1000 - URANGE( -3, 3 - victim->level, 6 ) * 50;
...

{
    ...
    else if ( dam <= 1000) { vs = " incredibly hard";vp = " incredibly hard";}

    else                   { vs = " amazingly hard"; vp = " amazingly hard";} 
...

    {
	    ...
	    else if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_MONK) &&

		victim->pcdata->powers[PMONK] > 3 && (dam-recover) > 0)
		...
		(dam-recover) > 0) victim->hit = victim->hit+number_range(1,((dam-recover)));
		...
		victim->pcdata->powers[NPOWER_CHIKYU] > 0 && (dam-recover) > 0)
...
*/
	...
	act("Your arms multiple, hitting $N many times.",ch,NULL,victim,TO_CHAR);

	act("$n's arms multiple, hitting $N many times.",ch,NULL,victim,TO_NOTVICT);

	act("$n's arms multiple, hitting you many times.",ch,NULL,victim,TO_VICT);
...

{
    ...
    if ( !IS_NPC(victim) && IS_IMMUNE(victim, IMM_BACKSTAB) && !IS_CLASS(ch, CLASS_NINJA) )
...

{
...

	{
	   ...
	   send_to_char("You cannot decapitate people in a non-decapitating room!\n\r", ch);
