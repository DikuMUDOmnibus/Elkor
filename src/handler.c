...

{
	...
	if (ch->in_room != NULL)
	    ...
	    send_to_char("Your steel shielding fades away.\n\r", ch);
...

{
	...
	act( "You are zapped by $p because of alignment and drop it.", ch, obj, NULL, TO_CHAR );

	act( "$n is zapped by $p because of alignment  and drops it.",  ch, obj, NULL, TO_ROOM );
	...
	act( "You are zapped by $p because it is too evil and drop it.", ch, obj, NULL, TO_CHAR );

	act( "$n is zapped by $p because it is too evil and drops it.",  ch, obj, NULL, TO_ROOM );
...
/* Only Ninjas wear ninja eq - Loki 
 ...
 */
	...
	act( "You are zapped by $p because you do not follow Lloth and drop it.", ch, obj, NULL, TO_CHAR );

	act( "$n is zapped by $p because they do not follow Lloth and drops it.",  ch, obj, NULL, TO_ROOM );
	...
	act( "You are zapped by $p because you are not a highlander and drop it.", ch, obj, NULL, TO_CHAR );

	act( "$n is zapped by $p because there are not a highlander and drops it.",  ch, obj, NULL, TO_ROOM );
...

{
    ...
    if ( vector & AFF_GODBLESS      ) strcat( buf, " godbless" );

	if ( vector & AFF_DROWBLESS      ) strcat( buf, " drowbless" );
